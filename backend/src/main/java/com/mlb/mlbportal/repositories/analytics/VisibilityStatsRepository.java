...
public interface VisibilityStatsRepository extends JpaRepository<VisibilityStats, LocalDate> {
    ...
    @Query(nativeQuery = true, value =
            "INSERT INTO t_visibility_stats (date, visualizations, new_users, deleted_users) " +
                    "VALUES (:date, 1, 0, 0) " +
    ...
    @Query(nativeQuery = true, value =
            "INSERT INTO t_visibility_stats (date, visualizations, new_users, deleted_users) " +
                    "VALUES (:date, 0, 0, 1) " +
                    "ON CONFLICT (date) DO UPDATE SET churnUsers = t_visibility_stats.deleted_users + 1"
    )
    void increaseDeletedUsers(@Param("date")LocalDate date);
}